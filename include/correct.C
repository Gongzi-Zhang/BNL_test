...
int main(int argc, char *argv[])
{
    ...
    if (!rootFiles.size())
    {
	cerr << FATAL << "no root files for run: " << run << endl;
	exit(4);
    }

    map<string, map<int, long long>> chRawADC;
    map<string, map<int, double>> chCorADC;
    map<string, map<int, double>> layerCorADC;
    double layerCorADC;
    double TS, preTS = 1e32;
    

    TTree *tout = new TTree("cor", "calibrate values");
	...
	for (auto gain : cali::gains)
	{
	    ...
	    tout->Branch(Form("Ch_%d_%s", ch, gain), &corValue[gain][ch]);
	}
    }
